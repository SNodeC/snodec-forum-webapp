...
#define SERVERROOT "/home/voc/projects/snodec/NDS/snodec-forum-webapp/public"
...
int main(int argc, char *argv[]) {
  ...
  staticServer.get("/users", [&mariaDbClient] APPLICATION(req, res) {
    std::cout << "Cookie-Value of \"SessionCookie\": "
              << req->cookie("sessionCookie") << std::endl;
    nlohmann::json *usersJson = new nlohmann::json;
    mariaDbClient.query(
        "SELECT user_id, username, password FROM users",
        [&res, usersJson](const MYSQL_ROW row) -> void {
          if (row != nullptr) {
            usersJson->push_back({{"user_id", row[0]},
                                  {"username", row[1]},
                                  {"password", row[2]}});
        ...
        [&res](const std::string &errorString,
  ...
  staticServer.post("/register", [&mariaDbClient] APPLICATION(req, res) {
    ...
    std::cout << Utils::GetFieldByName(req->body.data(), "username")
              << std::endl;
    std::cout << Utils::GetFieldByName(req->body.data(), "password")
              << std::endl;
    std::size_t password = std::hash<std::string>{}(
    ...
    *found = false;
    mariaDbClient.query(
        ...
        [found, &mariaDbClient, &res, &req,
         password](const MYSQL_ROW row) -> void {
          if (row != nullptr) {
              ...
              mariaDbClient.exec(
                  ...
                  [&mariaDbClient, &res, &req](void) -> void {
                    ...
                    mariaDbClient.affectedRows(
                        [&res, &mariaDbClient,
                         &req](my_ulonglong affectedRows) -> void {
                          VLOG(0)
                          ...
                          mariaDbClient.query(
                              "SELECT user_id FROM users WHERE username = '" +
                                  Utils::GetFieldByName(req->body.data(),
                                                        "username") +
                                  "';",
                              [&res, &mariaDbClient, sessionCookie,
                                  ...
                                  *userId = row[0];
                                  ...
                                  mariaDbClient.exec(
                                      ...
                                      "`userid`) VALUES ('" +
                                          ...
                                          "')",
                                      [&mariaDbClient, &res, sessionCookie,
                                       userId](void) -> void {
                                        VLOG(0) << "********** OnQuery 1: ";
                                        mariaDbClient.affectedRows(
                                            [&res, sessionCookie,
                                             userId](my_ulonglong affectedRows)
                                                -> void {
                                              VLOG(0) << "********** "
                                                         "AffectedRows 2: "
                                                      << affectedRows;
  ...
  staticServer.post("/login", [&mariaDbClient] APPLICATION(req, res) {
    ...
    mariaDbClient.query(
        "SELECT * FROM users WHERE username = '" +
            Utils::GetFieldByName(req->body.data(), "username") +
            "'AND password='" + std::to_string(password) + "';",
        [&res, usersJson, sessionCookie, &mariaDbClient,
         userId](const MYSQL_ROW row) -> void {
          if (row != nullptr) {
            ...
            *userId = row[0];
            ...
            if (!usersJson->is_null()) {
              ...
              mariaDbClient.exec(
                  "INSERT INTO `sessions`(`session_id`, `userid`) VALUES ('" +
                      sessionCookie + "','" + *userId + "')",
                  [&mariaDbClient, &res, sessionCookie, userId,
                   usersJson](void) -> void {
                    VLOG(0) << "********** OnQuery 1: ";
                    mariaDbClient.affectedRows(
                        [&res, sessionCookie, userId,
                         usersJson](my_ulonglong affectedRows) -> void {
                          VLOG(0)
                              << "********** AffectedRows 2: " << affectedRows;
  ...
  staticServer.get("/subtopics", [&mariaDbClient] APPLICATION(req, res) {
    ...
    mariaDbClient.query(
        "SELECT subtopics.subtopic_id, subtopics.title, subtopics.created_at, "
        "subtopics.userid, users.username FROM subtopics LEFT JOIN users ON "
        "subtopics.userid = users.user_id",
        [&res, subTopicsJson, &mariaDbClient](const MYSQL_ROW row) -> void {
          if (row != nullptr) {
            ...
            subTopicsJson->push_back({{"subtopic_id", subtopic_id},
                                      {"title", title},
          ...
          [userId, &req, &res, &mariaDbClient](const MYSQL_ROW row) -> void {
            if (row != nullptr) {
              ...
              *userId = row[0];
              ...
              req->body.push_back(0);
              std::cout << req->body.data() << std::endl;
              std::cout << Utils::GetFieldByName(req->body.data(), "title")
                        << std::endl;

              mariaDbClient.exec(
                  "INSERT INTO `subtopics`(`title`, `userid`) VALUES ('" +
  ...
  staticServer.get(
      "/subtopic/:subtopicid/threads", [&mariaDbClient] APPLICATION(req, res) {
        nlohmann::json *threadsJson = new nlohmann::json;
        mariaDbClient.query(
            "SELECT threads.thread_id, threads.title, threads.content, "
            "threads.created_at, threads.userid, threads.subtopicid, "
            "users.username FROM threads LEFT JOIN users ON threads.userid = "
            "users.user_id WHERE subtopicid = '" +
                req->params["subtopicid"] + "';",
            [&res, threadsJson](const MYSQL_ROW row) -> void {
              if (row != nullptr) {
                threadsJson->push_back({{"thread_id", row[0]},
                                        {"title", row[1]},
                                        {"content", row[2]},
                                        {"created_at", row[3]},
                                        {"userid", row[4]},
  ...
  staticServer.post(
      "/subtopic/:subtopicid/threads/new",
      [&mariaDbClient] APPLICATION(req, res) {
        std::string sessionCookie = req->cookie("sessionCookie");
        if (sessionCookie.empty()) {
          res->sendStatus(403);
        } else {
          std::string *userId = new std::string();
          mariaDbClient.query(
              "SELECT userid FROM sessions WHERE session_id = '" +
                  sessionCookie + "';",
              [userId, &req, &res,
               &mariaDbClient](const MYSQL_ROW row) -> void {
                if (row != nullptr) {
                  VLOG(0) << "Row Result 3: " << row[0];
                  *userId = row[0];
                } else {
                  ...
                  mariaDbClient.exec(
                      "INSERT INTO `threads`(`title`, `content`,`userid`, "
                      "`subtopicid`) VALUES ('" +
                          Utils::GetFieldByName(req->body.data(), "title") +
                          "','" +
                          Utils::GetFieldByName(req->body.data(), "content") +
                          "','" + *userId + "','" + req->params["subtopicid"] +
                          "');",
                      [&mariaDbClient, &res, userId](void) -> void {
                        VLOG(0) << "********** OnQuery 1: ";
                        mariaDbClient.affectedRows(
                            [&res, userId](my_ulonglong affectedRows) -> void {
                              VLOG(0) << "********** AffectedRows 2: "
                                      << affectedRows;
                              res->sendStatus(200);
                              delete userId;
                            },
  ...
  staticServer.get(
      "/subtopic/:subtopicid/threads/:threadid/posts",
      [&mariaDbClient] APPLICATION(req, res) {
        nlohmann::json *postsJson = new nlohmann::json;
        mariaDbClient.query(
            "SELECT posts.post_id, posts.content, posts.created_at, "
            "posts.userid, posts.threadid, users.username FROM posts LEFT JOIN "
            "users ON posts.userid=users.user_id WHERE threadid = '" +
                req->params["threadid"] + "';",
            [&res, postsJson](const MYSQL_ROW row) -> void {
              if (row != nullptr) {
                postsJson->push_back({{"post_id", row[0]},
                                      {"content", row[1]},
                                      {"created_at", row[2]},
                                      {"userid", row[3]},
                                      {"threadid", row[4]},
  ...
  staticServer.post(
      "/subtopic/:subtopicid/threads/:threadid/posts/new",
      [&mariaDbClient] APPLICATION(req, res) {
        std::string sessionCookie = req->cookie("sessionCookie");
        if (sessionCookie.empty()) {
          res->sendStatus(403);
               ...
               &mariaDbClient](const MYSQL_ROW row) -> void {
                if (row != nullptr) {
                  ...
                  *userId = row[0];
                  ...
                  std::cout << req->body.data() << std::endl;
                  mariaDbClient.exec(
                      "INSERT INTO `posts`(`content`,`userid`,`threadid`) "
                      "VALUES ('" +
                          Utils::GetFieldByName(req->body.data(), "content") +
  ...
  staticServer.post("/logout", [&mariaDbClient] APPLICATION(req, res) {
    ...
    if (sessionCookie.empty()) {
      ...
      mariaDbClient.exec(
          "DELETE FROM `sessions` WHERE session_id ='" + sessionCookie + "';",
  ...
  staticServer.get(
      "/home", [] APPLICATION(req, res) { res->redirect(308, "/index.html"); });
  ...
  staticServer.listen(
      ...
      [](const express::legacy::in::WebApp::SocketAddress &socketAddress,
        ...
        if (err != 0) {
          ...
          std::cout << "ForumApp is listening on " << socketAddress.toString()
                    << std::endl;
        }
      });
